...
public class BallsPerBoundaryMapper extends Mapper<LongWritable , Text, Text, Text>{
	...
	protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
		...
		if (season > 2015 && innings < 3) {			
			...
			if(!Objects.isNull(striker) && !striker.isEmpty() &&
					(runsInThatDelivery==4 || runsInThatDelivery==6) ) {
				boundary=1;
			}
			if(extras==0) {
				ball=1;
			...
			context.write(new Text(striker), new Text(boundary+"-"+ball+"-"+runsInThatDelivery));
		}
	}
}
